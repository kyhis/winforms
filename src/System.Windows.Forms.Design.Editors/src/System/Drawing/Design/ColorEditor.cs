            ...
            protected override IntPtr HookProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam)
            {
                switch (msg)
                {
                    case WindowMessages.WM_INITDIALOG:
                        ...
                        hwndCtl = UnsafeNativeMethods.GetDlgItem(hwnd, IDOK);
                        User32.EnableWindow(hwndCtl, BOOL.FALSE);
                        User32.SetWindowPos(
                            hwndCtl,
                            User32.HWND_TOP,
                    ...
                    case WindowMessages.WM_COMMAND:
                        switch (PARAM.LOWORD(wParam))
                        {
                            case COLOR_ADD:
                                ...
                                User32.PostMessageW(hwnd, User32.WindowMessage.WM_COMMAND, PARAM.FromLowHigh(IDOK, 0), UnsafeNativeMethods.GetDlgItem(hwnd, IDOK));
                                break;
                        }
                        break;
                }
